<script lang="ts">
    ...
    let changeMeteors = (num: number) => {
      ...
      const styles = [...new Array(num)].map(() => ({
        ...
        animationDuration: Math.floor(Math.random() * 10 + 2.9) + "s",
      }));
      meteorStyles = styles;
    };
    onMount(() => {
  ...
  {#each meteorStyles as style, idx}
    <span
      id="meteor-{idx+1}"
      class={cn(
        "pointer-events-none absolute left-1/2 top-1/2 size-[4.4px] rotate-[215deg] animate-meteor rounded-full bg-slate-500 shadow-[0_0_0_1px_#ffffff10]"
      )}
