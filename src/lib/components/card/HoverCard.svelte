    ...
    const items = [
      {
        id: 1,
        title: "Framer Motion",
        description: "Explore the new website that simplifies the creation of sophisticated dark mode components.",
    ...
    <AnimateSharedLayout>
      {#each items as one, i}
        <a
          ...
          class="relative flex flex-col gap-3 p-4 rounded-xl border border-white/10 transition-colors bg-transparent"
        >
          {#if hoverdIdx === one.id}
            <AnimatePresence let:item list={[{ key: hoverdIdx }]}>
              ...
              >
                <span
                  use:motion
                  class="absolute inset-0 z-0 block h-full w-full rounded-xl bg-neutral-800 cardHoverEffect"
                ></span>
              </Motion>
            </AnimatePresence>
          {/if}
